...
const SERVICES = [
  { icon: "Layers", title: "Беседки и террасы", desc: "Безрамное остекление открытых зон отдыха. Создаём уютное пространство, защищённое от ветра и осадков круглый год.", price: "от 35 000 ₽/м²" },
  { icon: "Home", title: "Частные дома", desc: "Панорамное остекление фасадов, зимних садов и веранд. Максимум естественного света без потери тепла.", price: "от 6 200 ₽/м²" },
  { icon: "Building2", title: "Балконы и лоджии", desc: "Безрамные системы для квартир — элегантно, надёжно, с шумоизоляцией до 42 дБ.", price: "от 3 800 ₽/м²" },
  ...
  { icon: "Flower2", title: "Зимние сады", desc: "Проектируем и остекляем зимние сады под ключ — тепло, светло и уютно в любое время года.", price: "от 7 500 ₽/м²" },
  { icon: "Tent", title: "Пергалы", desc: "Остекление пергол и навесов — открытые конструкции превращаем в комфортное всесезонное пространство.", price: "от 5 000 ₽/м²" },
  { icon: "Shield", title: "Цельностеклянные козырьки", desc: "Козырьки из закалённого стекла над входными группами, террасами и балконами. Надёжно и эстетично.", price: "от 8 000 ₽/м²" },
  { icon: "PanelTop", title: "Стеклянные ограждения", desc: "Цельностеклянные ограждения для лестниц, балконов и террас. Безопасное стекло класса триплекс.", price: "от 6 500 ₽/м²" },
];
...
const Index = () => {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden" style={{ background: "hsl(210,15%,6%)", color: "hsl(210,20%,92%)", fontFamily: "'Golos Text', sans-serif" }}>

      {/* NAV */}
      <header style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, transition: "all 0.3s", ...(scrolled ? { background: "rgba(12,16,22,0.96)", backdropFilter: "blur(12px)", borderBottom: "1px solid hsl(210,15%,14%)" } : {}) }}>
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {[["hero","Главная"],["services","Услуги"],["portfolio","Портфолио"],["calc","Расчёт"],["contacts","Контакты"]].map(([id, label]) => (
              <button key={id} onClick={() => scrollTo(id)} style={{ fontSize: 12, letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 500, color: "hsl(210,10%,55%)", background: "none", border: "none", cursor: "pointer", transition: "color 0.2s" }}
                onMouseEnter={e => (e.currentTarget.style.color = "#4db8d0")}
          ...
          <button onClick={() => scrollTo("contacts")} className="hidden md:block" style={{ background: "#4db8d0", color: "hsl(210,15%,6%)", padding: "9px 22px", fontSize: 12, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", border: "none", cursor: "pointer" }}>
            Связаться
          </button>

          <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden" style={{ background: "none", border: "none", cursor: "pointer", color: "hsl(210,20%,92%)" }}>
            <Icon name={menuOpen ? "X" : "Menu"} size={24} />
          </button>
        </div>

        {menuOpen && (
          <div style={{ background: "hsl(210,15%,9%)", borderTop: "1px solid hsl(210,15%,14%)", padding: "16px 24px", display: "flex", flexDirection: "column", gap: 12 }}>
            {[["hero","Главная"],["services","Услуги"],["portfolio","Портфолио"],["calc","Расчёт"],["contacts","Контакты"]].map(([id, label]) => (
              <button key={id} onClick={() => scrollTo(id)} style={{ textAlign: "left", fontSize: 13, letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 500, color: "hsl(210,10%,65%)", background: "none", border: "none", cursor: "pointer", padding: "4px 0" }}>
                {label}
              </button>
            ))}
          </div>
      ...
      <section id="hero" style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0 }}>
          ...
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, hsl(210,15%,6%) 30%, rgba(12,16,22,0.6) 70%, rgba(12,16,22,0.3))" }} />
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, hsl(210,15%,6%) 0%, transparent 50%)" }} />
        </div>
        ...
        <div className="relative max-w-6xl mx-auto px-6 w-full" style={{ paddingTop: 120, paddingBottom: 80 }}>
          <div style={{ maxWidth: 620 }}>
            <div className="animate-fade-in flex items-center gap-3 mb-6" style={{ animationDelay: "0.1s", opacity: 0 }}>
              <div style={{ height: 1, width: 40, background: "#4db8d0" }} />
              <span style={{ color: "#4db8d0", fontSize: 11, letterSpacing: "0.35em", textTransform: "uppercase", fontWeight: 500 }}>Безрамное остекление</span>
            </div>
            ...
            <h1 className="animate-fade-up" style={{ fontFamily: "'Cormorant', serif", fontSize: "clamp(42px,7vw,76px)", fontWeight: 300, lineHeight: 1.1, marginBottom: 24, animationDelay: "0.2s", opacity: 0 }}>
              Ваш дом без<br />
              <em style={{ color: "#4db8d0", fontStyle: "normal" }}>границ и рамок</em>
            </h1>
            ...
            <div className="animate-fade-up flex flex-wrap gap-4" style={{ animationDelay: "0.5s", opacity: 0 }}>
              <button onClick={() => scrollTo("calc")} style={{ background: "#4db8d0", color: "hsl(210,15%,6%)", padding: "13px 32px", fontWeight: 700, fontSize: 13, letterSpacing: "0.15em", textTransform: "uppercase", border: "none", cursor: "pointer" }}>
                Рассчитать стоимость
              </button>
              <button onClick={() => scrollTo("portfolio")} style={{ background: "transparent", color: "hsl(210,20%,85%)", padding: "13px 32px", fontWeight: 500, fontSize: 13, letterSpacing: "0.15em", textTransform: "uppercase", border: "1px solid rgba(77,184,208,0.3)", cursor: "pointer" }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = "#4db8d0"; e.currentTarget.style.color = "#4db8d0"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(77,184,208,0.3)"; e.currentTarget.style.color = "hsl(210,20%,85%)"; }}>
                Смотреть работы
              </button>
            </div>
          </div>
      ...
      <section id="services" className="py-24 max-w-6xl mx-auto px-6">
        <div style={{ marginBottom: 52 }}>
          <div className="flex items-center gap-3" style={{ marginBottom: 14 }}>
            <div style={{ height: 1, width: 40, background: "#4db8d0" }} />
            <span style={{ color: "#4db8d0", fontSize: 11, letterSpacing: "0.35em", textTransform: "uppercase" }}>Наши услуги</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {SERVICES.map((s) => (
            <div key={s.title} className="hover-lift" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(77,184,208,0.18)", padding: "32px", cursor: "default", transition: "transform 0.3s, box-shadow 0.3s, border-color 0.3s" }}
              onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(77,184,208,0.45)"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(77,184,208,0.18)"; }}>
              <div style={{ width: 48, height: 48, border: "1px solid rgba(77,184,208,0.3)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
                <Icon name={s.icon} fallback="Square" size={20} style={{ color: "#4db8d0" }} />
              </div>
              <h3 style={{ fontFamily: "'Cormorant', serif", fontSize: 24, fontWeight: 300, marginBottom: 10 }}>{s.title}</h3>
              <p style={{ color: "hsl(210,10%,53%)", fontSize: 14, lineHeight: 1.7, marginBottom: 14 }}>{s.desc}</p>
              <div style={{ color: "#4db8d0", fontSize: 13, fontWeight: 600, letterSpacing: "0.05em" }}>{s.price}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* PORTFOLIO */}
      <section id="portfolio" style={{ paddingTop: 80, paddingBottom: 80, borderTop: "1px solid hsl(210,15%,13%)" }}>
        <div className="max-w-6xl mx-auto px-6">
          <div style={{ marginBottom: 36 }}>
            <div className="flex items-center gap-3" style={{ marginBottom: 14 }}>
              <div style={{ height: 1, width: 40, background: "#4db8d0" }} />
              <span style={{ color: "#4db8d0", fontSize: 11, letterSpacing: "0.35em", textTransform: "uppercase" }}>Наши работы</span>
            </div>
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-5">
              <h2 style={{ fontFamily: "'Cormorant', serif", fontSize: "clamp(32px,5vw,52px)", fontWeight: 300 }}>Портфолио</h2>
              <div className="flex gap-2 flex-wrap">
                {TAGS.map((tag) => (
                  <button key={tag} onClick={() => setActiveTag(tag)} style={{ padding: "6px 16px", fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", fontWeight: 500, border: `1px solid ${activeTag === tag ? "#4db8d0" : "hsl(210,15%,20%)"}`, background: activeTag === tag ? "#4db8d0" : "transparent", color: activeTag === tag ? "hsl(210,15%,6%)" : "hsl(210,10%,55%)", cursor: "pointer", transition: "all 0.2s" }}>
                    {tag}
                  </button>
                ))}
              </div>
            ...
            {filtered.map((item) => (
              <div key={item.id} onClick={() => setLightbox(item.id)} style={{ position: "relative", cursor: "pointer", overflow: "hidden" }}
                ...
                <div className="portfolio-overlay" style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(12,16,22,0.95) 0%, rgba(12,16,22,0.1) 60%, transparent)", opacity: 0, transition: "opacity 0.3s", display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: 20 }}
                  ...
                  <div style={{ color: "#4db8d0", fontSize: 11, letterSpacing: "0.25em", textTransform: "uppercase", marginBottom: 4 }}>{item.tag}</div>
                ...
                <div style={{ position: "absolute", top: 12, right: 12, width: 32, height: 32, background: "#4db8d0", display: "flex", alignItems: "center", justifyContent: "center", opacity: 0, transition: "opacity 0.3s" }}>
          ...
          <div onClick={() => setLightbox(null)} style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(12,16,22,0.97)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
            <div onClick={(e) => e.stopPropagation()} style={{ position: "relative", maxWidth: 900, width: "100%" }}>
              ...
              <div style={{ marginTop: 16, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  <div style={{ color: "#4db8d0", fontSize: 11, letterSpacing: "0.3em", textTransform: "uppercase", marginBottom: 4 }}>{item.tag}</div>
                  <div style={{ fontFamily: "'Cormorant', serif", fontSize: 24, fontWeight: 300 }}>{item.title}</div>
      ...
      {/* CALCULATOR */}
      <section id="calc" style={{ paddingTop: 80, paddingBottom: 80, borderTop: "1px solid hsl(210,15%,13%)" }}>
        <div className="max-w-6xl mx-auto px-6">
          <div style={{ marginBottom: 52 }}>
            <div className="flex items-center gap-3" style={{ marginBottom: 14 }}>
              <div style={{ height: 1, width: 40, background: "#4db8d0" }} />
              <span style={{ color: "#4db8d0", fontSize: 11, letterSpacing: "0.35em", textTransform: "uppercase" }}>Онлайн-расчёт</span>
            </div>
            <h2 style={{ fontFamily: "'Cormorant', serif", fontSize: "clamp(32px,5vw,52px)", fontWeight: 300 }}>Рассчитайте стоимость</h2>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(77,184,208,0.2)", padding: 36 }}>
              {!submitted ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {["Беседки","Дома","Балконы","Коммерция"].map((t) => (
                        <button key={t} onClick={() => setForm({ ...form, type: t })} style={{ padding: "10px 8px", fontSize: 13, border: `1px solid ${form.type === t ? "#4db8d0" : "hsl(210,15%,18%)"}`, background: form.type === t ? "rgba(77,184,208,0.1)" : "transparent", color: form.type === t ? "#4db8d0" : "hsl(210,10%,55%)", cursor: "pointer", letterSpacing: "0.05em", transition: "all 0.2s" }}>
                          {t}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <input type="number" placeholder="Например: 24" value={form.area} onChange={(e) => setForm({ ...form, area: e.target.value })} style={{ width: "100%", background: "hsl(210,15%,10%)", border: "1px solid hsl(210,15%,18%)", outline: "none", padding: "12px 16px", color: "hsl(210,20%,92%)", fontSize: 15, boxSizing: "border-box" }} onFocus={e => (e.currentTarget.style.borderColor = "#4db8d0")} onBlur={e => (e.currentTarget.style.borderColor = "hsl(210,15%,18%)")} />
                  </div>

                  {cost && (
                    <div style={{ border: "1px solid rgba(77,184,208,0.3)", background: "rgba(77,184,208,0.07)", padding: 20 }}>
                      <div style={{ fontSize: 11, letterSpacing: "0.25em", textTransform: "uppercase", color: "#4db8d0", marginBottom: 8 }}>Предварительная стоимость</div>
                      <div style={{ fontFamily: "'Cormorant', serif", fontSize: 32, fontWeight: 300 }}>{cost.min} — {cost.max} ₽</div>
                      <div style={{ fontSize: 12, color: "hsl(210,10%,44%)", marginTop: 6 }}>Точная цена — после бесплатного замера</div>
                    </div>
                  )}

                  <div style={{ borderTop: "1px solid hsl(210,15%,14%)", paddingTop: 20, display: "flex", flexDirection: "column", gap: 14 }}>
                    <div>
                      ...
                      <input type="text" placeholder="Имя" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={{ width: "100%", background: "hsl(210,15%,10%)", border: "1px solid hsl(210,15%,18%)", outline: "none", padding: "12px 16px", color: "hsl(210,20%,92%)", fontSize: 15, boxSizing: "border-box" }} onFocus={e => (e.currentTarget.style.borderColor = "#4db8d0")} onBlur={e => (e.currentTarget.style.borderColor = "hsl(210,15%,18%)")} />
                    </div>
                    <div>
                      ...
                      <input type="tel" placeholder="+7 (___) ___-__-__" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} style={{ width: "100%", background: "hsl(210,15%,10%)", border: "1px solid hsl(210,15%,18%)", outline: "none", padding: "12px 16px", color: "hsl(210,20%,92%)", fontSize: 15, boxSizing: "border-box" }} onFocus={e => (e.currentTarget.style.borderColor = "#4db8d0")} onBlur={e => (e.currentTarget.style.borderColor = "hsl(210,15%,18%)")} />
                    </div>
                    <button onClick={() => { if (form.name && form.phone) setSubmitted(true); }} disabled={!form.name || !form.phone} style={{ width: "100%", background: form.name && form.phone ? "#4db8d0" : "rgba(77,184,208,0.3)", color: "hsl(210,15%,6%)", padding: "14px", fontWeight: 700, fontSize: 13, letterSpacing: "0.15em", textTransform: "uppercase", border: "none", cursor: form.name && form.phone ? "pointer" : "not-allowed", transition: "opacity 0.2s" }}>
                ...
                <div style={{ textAlign: "center", padding: "40px 0" }}>
                  <div style={{ width: 64, height: 64, border: "1px solid #4db8d0", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 24px" }}>
                    <Icon name="Check" size={28} style={{ color: "#4db8d0" }} />
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
              ...
              {[
                ...
                <div key={step.n} style={{ display: "flex", gap: 20 }}>
                  <div style={{ fontFamily: "'Cormorant', serif", fontSize: 40, color: "#4db8d0", opacity: 0.38, fontWeight: 300, lineHeight: 1, flexShrink: 0, width: 44 }}>{step.n}</div>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 14, letterSpacing: "0.05em", marginBottom: 4, color: "hsl(210,20%,86%)" }}>{step.title}</div>
                    <div style={{ color: "hsl(210,10%,50%)", fontSize: 14, lineHeight: 1.7 }}>{step.desc}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* CONTACTS */}
      <section id="contacts" style={{ paddingTop: 80, paddingBottom: 80, borderTop: "1px solid hsl(210,15%,13%)" }}>
        <div className="max-w-6xl mx-auto px-6">
          <div style={{ marginBottom: 48 }}>
            <div className="flex items-center gap-3" style={{ marginBottom: 14 }}>
              <div style={{ height: 1, width: 40, background: "#4db8d0" }} />
              <span style={{ color: "#4db8d0", fontSize: 11, letterSpacing: "0.35em", textTransform: "uppercase" }}>Контакты</span>
            </div>
            ...
            {[
              ...
              <div key={c.title} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(77,184,208,0.18)", padding: 28 }}>
                <div style={{ width: 42, height: 42, border: "1px solid rgba(77,184,208,0.3)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
                  <Icon name={c.icon} fallback="Square" size={18} style={{ color: "#4db8d0" }} />
                </div>
          ...
          <div style={{ background: "rgba(77,184,208,0.06)", border: "1px solid rgba(77,184,208,0.25)", padding: "40px 48px", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 20 }}>
            <div>
              ...
              <p style={{ color: "hsl(210,10%,50%)", fontSize: 14 }}>Оставьте заявку и мы перезвоним в течение 30 минут</p>
            </div>
            <button onClick={() => scrollTo("calc")} style={{ flexShrink: 0, background: "#4db8d0", color: "hsl(210,15%,6%)", padding: "13px 32px", fontWeight: 700, fontSize: 13, letterSpacing: "0.15em", textTransform: "uppercase", border: "none", cursor: "pointer", whiteSpace: "nowrap" }}>
              Заказать замер
            </button>
          </div>
        </div>
      ...
      {/* FOOTER */}
      <footer style={{ borderTop: "1px solid hsl(210,15%,12%)", padding: "28px 24px" }}>
          ...
          <div className="flex gap-6">
            {[["Услуги","services"],["Портфолио","portfolio"],["Контакты","contacts"]].map(([l, id]) => (
              <button key={id} onClick={() => scrollTo(id)} style={{ fontSize: 11, color: "hsl(210,10%,38%)", background: "none", border: "none", cursor: "pointer", letterSpacing: "0.15em", textTransform: "uppercase", transition: "color 0.2s" }}
                onMouseEnter={e => (e.currentTarget.style.color = "#4db8d0")}
                onMouseLeave={e => (e.currentTarget.style.color = "hsl(210,10%,38%)")}>
                {l}
              </button>
            ))}
          </div>
